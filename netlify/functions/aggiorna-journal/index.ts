...
const AUTHOR_COL_ENV = process.env.POEMS_AUTHOR_COLUMN || 'user_id'      // es. 'user_id' | 'profile_id'
...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    const authorId: string | undefined = rec[AUTHOR_COL_ENV] || rec.author_id || rec.user_id || rec.profile_id
    if (!authorId) {
    ...
    // -------- 1) Conta poesie (fallback: AUTHOR_COL_ENV → profile_id → user_id)
    log('counting poems for author', { authorId, AUTHOR_COL_ENV })
    ...
    async function tryCount(col: string) {
      ...
      return { col, count: count ?? 0, error }
    }
    ...
    let res = await tryCount(AUTHOR_COL_ENV)
    attempts.push({ col: AUTHOR_COL_ENV, count: res.count, err: res.error && {
      message: res.error.message, code: (res.error as any).code, details: (res.error as any).details
    }})
    if (!res.error) {
      poemsCount = res.count
    } else {
      res = await tryCount('profile_id')
      attempts.push({ col: 'profile_id', count: res.count, err: res.error && {
        message: res.error.message, code: (res.error as any).code, details: (res.error as any).details
      }})
      if (!res.error) {
        activeCol = 'profile_id'
        poemsCount = res.count
      } else {
        res = await tryCount('user_id')
        attempts.push({ col: 'user_id', count: res.count, err: res.error && {
          message: res.error.message, code: (res.error as any).code, details: (res.error as any).details
        }})
        if (!res.error) {
          activeCol = 'user_id'
          ...
          logErr('supabase count error on all columns', attempts)
          throw new Error(`Supabase count error: ${JSON.stringify({ attempts })}`)
        }
      }
    }

    log('count ok', { activeCol, poemsCount, attempts })
    ...
    // -------- 2) Leggi poesie (usando activeCol)
    log('fetching poems list', { authorId, activeCol })
    ...
    if (!poems || poems.length === 0) {
      ...
      return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'Nessuna poesia trovata per autore', authorId, activeCol, reqId }) }
    }

    // -------- 3) Profilo/QR preesistenti
    ...
    if (dryRun) {
      ...
      return {
        ...
        headers: CORS,
        body: JSON.stringify({
          ok: true, dryRun: true, authorId, poemsCount, activeCol, hasQrColumn, hadExistingQr: !!existingQr, envOk, reqId
        })
      }
    }

    // -------- 5) OpenAI (con fallback)
    ...
    if (hasQrColumn && publicUrl && !existingQr) {
      ...
      if (qr) { qrToSave = qr; log('QR generated') } else { log('QR generation failed (soft)') }
    }
    ...
    log('updating profile', { includeQr: !!qrToSave })
    ...
    log('DONE OK', { authorId, poemsCount, activeCol })
    return {
      statusCode: 200,
      headers: CORS,
      body: JSON.stringify({
