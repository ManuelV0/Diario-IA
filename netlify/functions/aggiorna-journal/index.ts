...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    // -------- 1) Conta poesie (fallback: AUTHOR_COL_ENV → profile_id → user_id) SENZA head:true
    log('counting poems for author', { authorId, AUTHOR_COL_ENV })

    async function tryCountAndProbe(col: string) {
      // select senza head:true per evitare bug di PostgREST con HEAD
    ...
    let r = await tryCountAndProbe(AUTHOR_COL_ENV)
    attempts.push(r)
    if (!r.error) {
      poemsCount = r.count
    } else {
      r = await tryCountAndProbe('profile_id')
      attempts.push(r)
      if (!r.error) {
        activeCol = 'profile_id'
        poemsCount = r.count
      } else {
        r = await tryCountAndProbe('user_id')
        attempts.push(r)
        if (!r.error) {
          activeCol = 'user_id'
          poemsCount = r.count
    ...
    log('count ok (no-HEAD)', { activeCol, poemsCount, attempts })
    ...
    if (!poems || poems.length === 0) {
      log('no poems found for author', { authorId, activeCol })
